...
namespace learn.Controllers
{
    public class HomeController : Controller
    {
        ...
        public JsonResult main()
        {
            ...
            string url = string.Empty;
            var res = Simples(out url);
            js.Data = res;
            return js;
        }

        public string Simples(out string URL)
        {
        ...
        public string Simple()
        {
            ...
            config.set("id", "author123456");

            JsonObject request = new JsonObject();
            request.set("mode", "activity_list");
            request.set("title.show", true);
            request.set("user", config);
            // Instantiate Init class
            Init init = new Init(service, security, secret, request);

            // Call the generate() method to retrieve a JavaScript object
            string str = init.generate();
            return str;
        }
